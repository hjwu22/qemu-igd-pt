...
static uint64_t endianness_convert(endianness_buf *buf, unsigned size)
{
	uint64_t data = 0;
                              ...
                              hwaddr addr, unsigned size)
{
	...
	data = endianness_convert(&buf, size);
                                  ...
                                  uint32_t val, int len)
{
    ...
    DPRINTF("%s(%04x:%02x:%02x.%x, @0x%x, 0x%x, len=0x%x)\n", __func__,
            0, 0, 3, 0, addr, val, len);

    /* Write everything to VFIO, let it filter out what we can't write */
    if (pwrite(vdev->fd, &val_le, len, vdev->config_offset + addr) != len) {
        error_report("%s(%04x:%02x:%02x.%x, 0x%x, 0x%x, 0x%x) failed: %m",
                     __func__, 0, 0, 3, 0, addr, val, len);
    }
...
static bool vfio_listener_skipped_section(MemoryRegionSection *section)
{
    return !memory_region_is_ram(section->mr) ||
           ...
           section->offset_within_address_space & 0xFFFFFF||
                                     ...
                                     MemoryRegionSection *section)
{
    ...
    //end = (section->offset_within_address_space + int128_getsection->size) &
          TARGET_PAGE_MASK;
...
static int vfio_setup_msi(VFIODevice *vdev, int pos)
{
	...
	//set msi related configs of emulated device
...
static int is_hda_available(void)
{
	char path[PATH_MAX];
	struct stat st;
	DPRINTF("%s\n", __func__);
...
static int is_vfio_device_attached(VFIOGroup *group, VFIODevice *vdev)
{
	...
	QLIST_FOREACH(pvdev, &group->device_list, next) {
        if (pvdev->host.domain == vdev->host.domain &&
            ...
            error_report("vfio: error: hda is already attached");
...
static int vfio_hda_initfn(PCIDevice *pdev)
{
    VFIODevice *vdev = DO_UPCAST(VFIODevice, pdev, pdev);
	VFIOGroup *group;
	int groupID;
	...
	if (ret) {
        error_report("vfio: %s: failed to get HDA", __func__);
        goto out_put_group;
        return ret;
    }
    ...
    //memset(vdev->emulated_config_bits + PCI_ROM_ADDRESS, 0xff, 3);
	//vdev->emulated_config_bits[PCI_ROM_ADDRESS] = 0xFFFFFFFE;
...
static void vfio_hda_exitfn(PCIDevice *pdev)
{
    VFIODevice *vdev = DO_UPCAST(VFIODevice, pdev, pdev);
...
static void vfio_pci_dev_class_init(ObjectClass *klass, void *data)
{
    ...
    dc->desc = "VFIO-based Intel HD Audio Controller";
    set_bit(DEVICE_CATEGORY_MISC, dc->categories);
    pdc->init = vfio_hda_initfn;
    pdc->exit = vfio_hda_exitfn;
